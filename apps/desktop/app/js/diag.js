...
export function initDiag({
  ...
  // WebSocket
  try {
    const s = window.io(wsUrl, { withCredentials: true, path: "/socket.io" });
  ...
  window.$diag = {
    createGame: (hostName) =>
      fetch(`${apiBase}/games`, {
        ...
        .then(r => r.json())
        .then(x => (console.log("[DIAG] createGame ->", x), x)),

    joinGame: (code, name) =>
      fetch(`${apiBase}/games/${encodeURIComponent(code)}/join`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name }),
      })
        .then(r => r.json())
        .then(x => (console.log("[DIAG] joinGame ->", x), x)),
  };
}
